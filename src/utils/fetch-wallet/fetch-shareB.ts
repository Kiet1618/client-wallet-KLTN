...
import { kCombinations, keccak256, thresholdSame } from '..';
import BN from 'bn.js';
import { ec as EC } from 'elliptic';
export const ec = new EC('secp256k1');


const pingNode = async (endpoint: string) => {
  try {
...
const configFetch = async () => {
  ...
  if (nodeEndpoints.length < 2) throw new Error('Not enough nodes');
...
export const getPrivateKey = async (input: GetPrivateKeyRequest): Promise<{ data?: GetPrivateKeyResponse, error?: ErrorApi }> => {
  try {
    ...
    const { nodeEndpoints, indexes } = await configFetch();
    const signatures = [];
